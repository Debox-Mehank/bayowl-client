...
const Pricing = ({ free }: { free: boolean }) => {
  ...
  const handleProceed = async () => {
    if (selectedServiceFinal) {
      ...
      setLoading(true);
      console.log(
        selectedServiceFinal.addOn.map((el) => ({
          type: el.type,
          value: el.value,
          main: el.main,
        }))
      ...
      const { data, error } = await initiatePaymentQuery({
        variables: {
          service: {
            ...
            price: total,
            allAddOns: selectedServiceFinal.addOn.map((el) => ({
              type: el.type,
              value: el.value,
              main: el.main,
            })),
      ...
      if (data.initiatePayment === "free service") {
        toast.success("Service added to your account successfully");
          ...
          <div className="max-w-7xl mx-auto lg:py-8 relative">
            ...
            {!selectedServiceFinal && (
              <div className=" mt-6 text-center font-bold text-xl mb-8 mx-auto space-y-2 md:bg-darkBlue/30 md:backdrop-blur-xl z-20 px-4 pb-3">
                {selectedService && selectedService[0].subService
                  ? selectedService[0].serviceName
                  : selectedService && selectedService[0].subCategory}
              </div>
            )}
            {selectedService &&
                          ...
                          {_.uniqBy(types, (el) => el.name).map((el, elIdx) => {
                            return (
                              <tr key={el.id} className="text-center">
                                <th
                                  className={`py-5 px-6 text-sm text-white text-left ${
                                    el.name.includes("Independent") &&
                ...
                >
                  <div className="mx-auto w-full space-y-8 rounded-lg py-12 bg-blueGradient-2/30 backdrop-blur-lg relative">
                    ...
                    <div className="text-xl space-y-3">
                      ...
                      <div>
                        Track Count Limit :{" "}
                        {selectedServiceFinal.inputTrackLimit}
                      </div>
                      <div>
                        Upload File Format:{" "}
