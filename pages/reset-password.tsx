...
import { MeDocument, MeQuery, useRegisterLazyQuery, } from "../graphql/generated/graphql";
...
const ResetPass: NextPage = () => {
    ...
    const handleSubmit = async () => {
        ...
        if (details.newPass !== details.confirmNewPass) {
            toast.error("New password and confirmed password does not match. Please try again.")
            return;
        }
        ...
        try {
            ...
            const { data, error } = await registerQuery({
                // variables: {
                //   password: details.newPass.toString().trim(),
                // },
            ...
            setLoading(false);
            setDetails({ newPass: "", confirmNewPass: "" })
            toast.success(
                "Congragulations your password has been reset.",
                { duration: 3000 }
            );
    ...
    return (
        <div className="flex flex-col md:flex-row w-full h-screen text-white">
            ...
            <div className="w-full min-h-2/3 md:h-full md:w-1/2 grid place-items-center bg-darkBlue relative md:p-0">
                ...
                {loading ? <Loader /> :
                    <div className="space-y-4 text-center z-10 px-4 relative w-full md:px-12 max-w-lg">
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            viewBox="0 0 504.04 517.13"
                            className="fill-white mb-8 mx-auto hover:fill-primary cursor-pointer transition-colors duration-300"
                            height={150}
                            width={150}
                        >
                ...
                before:content-[""] before:flex-1 before:border-b-2 before:border-solid before:m-auto before:ml-2.5'
                        ...
                        <div className="space-y-6">
                            <div className="w-full bg-black/30 flex items-center rounded-xl pr-2 ">
                                <input
                                    ...
                                    onChange={(e) => setDetails(prev => ({ ...prev, newPass: e.target.value }))}
                                />
                                {
                                    !showPassword ? (
                            ...
                            <div className="w-full bg-black/30 flex items-center rounded-xl pr-2 ">
                                <input
                                    ...
                                    value={details.confirmNewPass}
                                    onChange={(e) => setDetails(prev => ({ ...prev, confirmNewPass: e.target.value }))}
                                />
                                {
                                    !showConfPassword ? (
                        ...
                        <button
                            ...
                            className="w-full bg-black/70 filter backdrop-blur-3xl font-bold text-lg rounded-xl px-4 py-3 cursor-pointer transition-all hover:scale-105 hover:bg-black/90 duration-300">
                            Submit
                        </button>
                    </div>}
            </div>
