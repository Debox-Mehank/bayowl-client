...
export class Uploader {
  ...
  async initialize(fileName: string) {
    ...
    const [multipartUrlQuerys] = useGetMultipartPreSignedUrlsLazyQuery();
    try {
      const initializeReponse = await initFileUploadQuery({
        variables: { fileName: fileName + ".zip" },
      });
      //   api.request({
      ...
      const AWSFileDataOutput = initializeReponse.data?.initFileUpload;
      ...
      const urlsResponse = await multipartUrlQuerys({
        variables: {
          fileId: AWSMultipartFileDataInput.fileId,
          fileKey: AWSMultipartFileDataInput.fileKey,
          parts: AWSMultipartFileDataInput.parts,
        },
      });
      //   await api.request({
      ...
      const newParts = urlsResponse.data?.getMultipartPreSignedUrls!;
  ...
  onProgress(onProgress: (props?: any) => void) {
